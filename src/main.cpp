...
#define chk(cond) chk_impl(cond, __FILE__, __LINE__, __func__)

static inline void chk_impl(bool result, const char *file, int line, const char *func) {
  if (!result) {
...
static inline void chk_impl(VkResult result, const char *file, int line, const char *func) {
...
int main(int argc, char *argv[]) {
  ...
  chk(checkValidationLayerSupport());
  ...
  // get platform specific instance extensions
  ...
  if (instanceExtensions == NULL) {
    std::cerr << "Failed to get instance extensions" << std::endl;
  }

  // create vulkan instance
  const char *layerName = VALIDATION_LAYER_NAME;
  ...
  // get the list of GPUs that are capable of doing vulkan
  ...
  std::cerr << "Selected device: " << deviceProperties.properties.deviceName << std::endl;

  // get the "queue family" for doing graphics
  ...
  for (size_t i = 0; i < queueFamilies.size(); i++) {
    if (queueFamilies[i].queueFlags & VK_QUEUE_GRAPHICS_BIT) {
      // we found the right queue family!
      queueFamily = i; break; }
  }

  // make sure that we can present something to the screen
  chk(SDL_Vulkan_GetPresentationSupport(instance, devices[deviceIndex], queueFamily));
  
  // get a handle to the gpu (logical device)
  ...
  chk(vkCreateDevice(devices[deviceIndex], &deviceCreateInfo, nullptr, &device));
  ...
  chk(vmaCreateAllocator(&allocatorCreateInfo, &allocator));
  ...
  // create the swapchain that holds the buffers we render to and present
  // the combo of VK_FORMAT_B8G8R8A8_SRGB + VK_COLORSPACE_SRGB_NONLINEAR_KHR
  // is guaranteed to be available everywhere
  // VK_PRESENT_MODE_FIFO_KHR is also available everywhere
  ...
  VkSwapchainCreateInfoKHR swapchainCreateInfo {
    ...
    .imageExtent {
      .width = std::clamp( // surfaceCaps.currentExtent.width,
        static_cast<uint32_t>(windowSize.x),
        surfaceCaps.minImageExtent.width,
        surfaceCaps.maxImageExtent.width
      ),
      .height = std::clamp( // surfaceCaps.currentExtent.height,
  ...
  chk(vkCreateSwapchainKHR(device, &swapchainCreateInfo, nullptr, &swapchain));
  ...
  chk(vmaCreateImage(allocator, &depthImageCreateInfo, &allocCreateInfo, &depthImage, &depthImageAllocation, nullptr));
  ...
  VkDeviceSize indexCount = shapes[0].mesh.indices.size();
  std::cerr << "loaded " << indexCount << " instances" << std::endl;
  ...
  VmaAllocationCreateInfo vBufferAllocCreateInfo {
  ...
  chk(vmaCreateBuffer(
    allocator,
    &bufferCreateInfo,
    &vBufferAllocCreateInfo,
    &vBuffer,
    &vBufferAllocation,
    &vBufferAllocInfo
  ));

  // copy mesh into gpu buffer
  ...
  for (uint32_t i = 0; i < maxFramesInFlight; i++) {
    chk(vkCreateFence(device, &fenceCreateInfo, nullptr, &fences[i]));
    chk(vkCreateSemaphore(device, &semaphoreCreateInfo, nullptr, &presentSemaphores[i]));
  };
  renderSemaphores.resize(swapchainImages.size());
  for (auto &semaphore : renderSemaphores) {
    chk(vkCreateSemaphore(device, &semaphoreCreateInfo, nullptr, &semaphore));
  }
  ...
  chk(vkCreateCommandPool(device, &commandPoolCreateInfo, nullptr, &commandPool));

  VkCommandBufferAllocateInfo cbAllocCreateInfo {
    .sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO,
    .commandPool = commandPool,
    .commandBufferCount = maxFramesInFlight,
  };
  chk(vkAllocateCommandBuffers(device, &cbAllocCreateInfo, commandBuffers.data()));
  ...
  // the following are not actually used, but are required when creating
  // a pipeline and need to have sane values.
  ...
  // finally create the pipeline (WHOLY MOLY VULKAN MOMENT)
  ...
  while (!quit) {
    ...
    for (uint32_t i = 0; i < 3; i++) {
      auto instancePos = glm::vec3(((float)i - 1.0f) * 3.0f, 0.0f, 0.0f);
    ...
    if (updateSwapchain) {
      ...
      swapchainCreateInfo.imageExtent = { .width = static_cast<uint32_t>(windowSize.x), .height = static_cast<uint32_t>(windowSize.y)};
      chk(vkCreateSwapchainKHR(device, &swapchainCreateInfo, nullptr, &swapchain));
      for (auto i = 0; i < imageCount; i++) {
      ...
      depthImageCreateInfo.extent = { .width = static_cast<uint32_t>(windowSize.x), .height = static_cast<uint32_t>(windowSize.y), .depth = 1 };
      VmaAllocationCreateInfo allocCI{ .flags = VMA_ALLOCATION_CREATE_DEDICATED_MEMORY_BIT, .usage = VMA_MEMORY_USAGE_AUTO };
      chk(vmaCreateImage(allocator, &depthImageCreateInfo, &allocCI, &depthImage, &depthImageAllocation, nullptr));
